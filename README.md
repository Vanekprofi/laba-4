...
##Алгоритм решения  
Реализовать класс Box, содержащий поле для хранение обьекта. Реализовать методы для вложения обьекта, взятия и очищения обьекта, проверка на заполненность коробки.
...
##Алгоритм решения  
Реализовать класс Storage с полем value. Реализовать метод getValue(T alternative), который будет возвращать значение если оно не null, иначе альтернативу.
...
Алгоритм решения
Создаём интерфейс Test с методом boolean test(T t). Реализуем метод filter, который проверяет каждый элемент и добавляет его в результат при успехе.
...
Алгоритм решения
Создаём интерфейс Reducer с методом reduce(T accumulator, T current). Реализуем метод reduceList, который сводит список к одному значению.
